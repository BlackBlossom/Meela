...
(function() {
  ...
  // Helper: update in-memory cart from localStorage (used by storage event)
  function refreshCartFromStorage() {
    try {
      cart = JSON.parse(localStorage.getItem('meelaCart')) || [];
    } catch (e) {
      console.warn('[CART-FIX] Failed to parse meelaCart from localStorage', e);
      cart = [];
    }
    updateCartCount();
    // updateCartDisplay may not exist on some pages (no cart UI), so guard call
    try { updateCartDisplay(); } catch (err) { /* ignore if not present */ }
    console.log('[CART-FIX] Cart refreshed from storage', cart);
  }

  // Listen for storage events to sync cart across pages/tabs
  window.addEventListener('storage', function(e) {
    if (!e) return;
    if (e.key === 'meelaCart') {
      console.log('[CART-FIX] storage event: meelaCart changed');
      refreshCartFromStorage();
    }
    if (e.key === 'meelaUserAddress') {
      // Address changed on another page - update UI if present
  ...
  function updateCartDisplay() {
    ...
    if (cart.length === 0) {
      ...
      cart.forEach((item, index) => {
        ...
        cartHTML += '<p style="font-size:14px;margin-bottom:8px;">Price: ' + formatPrice(item.price) + '</p>';
        ...
        cartHTML += '</div>';
        cartHTML += '<p style="font-size:14px;margin-top:8px;">Subtotal: ' + formatPrice(itemTotal) + '</p>';
        cartHTML += '</div>';
      ...
      // Show subtotal
      cartHTML += '<div style="display:flex;justify-content:space-between;font-size:16px;margin-bottom:10px;">';
      cartHTML += '<span>Subtotal:</span><span>' + formatPrice(total) + '</span>';
      cartHTML += '</div>';
      
      // Show discount if applicable
      if (discount > 0) {
        cartHTML += '<div style="display:flex;justify-content:space-between;font-size:16px;margin-bottom:10px;color:#27ae60;">';
        cartHTML += '<span>Discount (20%):</span><span>-' + formatPrice(discount) + '</span>';
        cartHTML += '</div>';
      }
      
      // Show final total
      cartHTML += '<div style="display:flex;justify-content:space-between;font-weight:600;font-size:18px;margin-bottom:20px;padding-top:10px;border-top:1px solid #eee;">';
      cartHTML += '<span>Total:</span><span>' + formatPrice(finalTotal) + '</span>';
      cartHTML += '</div>';
  ...
  // Generate custom order ID in format ORDER-123456-HAIROIL
  function generateOrderId() {
    const randomNum = Math.floor(100000 + Math.random() * 900000);
    return 'ORDER-' + randomNum + '-HAIROIL';
  }
  ...
  function savePurchaseToFirebase() {
    ...
    window.db.collection('purchaseHistory')
      ...
      .then(function(docRef) {
        console.log('[FIREBASE] Purchase saved with Order ID:', customOrderId);
        ...
        // Show success popup with custom order ID
        showSuccessPopup(customOrderId);
